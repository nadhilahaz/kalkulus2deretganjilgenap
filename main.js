function hitung() {
  ...
  if (deret === "ganjil") {
    ...
    for (var i = 1; i <= batas; i++) {
      ...
      deret_str += deret_genap + " "; // syntax untuk menambahkan 'deret_genap' ke variabel 'deret_str' yang berisi deret bilangan genap, diikuti dengan spasi //
      sum +=
...
function reset() {
  ...
  // Mendapatkan referensi ke elemen tombol keluar
  var keluarBtn = document.getElementById("keluarBtn");

  // Menambahkan event listener untuk mengarahkan pengguna ke halaman lain ketika tombol diklik
  keluarBtn.addEventListener("click", function () {
    window.location.href = "materi.html"(); // Mengarahkan ke halaman kosong
    // Atau bisa juga menggunakan window.close() untuk menutup tab/window
  });
}
